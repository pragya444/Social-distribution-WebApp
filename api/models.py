...
class User(AbstractBaseUser, PermissionsMixin):
    ...
    profile_picture = models.CharField(max_length=255, blank=True, null=True, default="")
...
class Entry(models.Model):
    ...
    url = models.CharField(max_length=255, blank=True, db_index=True, null=True, unique=True)
    ...
    '''
    Pragya's method to save FQID and URL.
    def save(self, *args, **kwargs):
    ...
    def save(self, *args, **kwargs):
       ...
       # id is generated by default=generate_id, so it's already set on the instance
        if not self.url:
             self.url = f"{base}/api/authors/{self.author_id}/entries/{self.id}"

        if not self.fqid:
          # reuse url so they always match
             self.fqid = self.url
        # or, if you prefer:
        # self.fqid = f"{base}/api/authors/{self.author_id}/entries/{self.id}"
